...
// if (playGame) {
//   // run loop only if Ok was selected
...
//                   ? "Player Wins"
//                   : "Tie"
//               : "invalid input"; // after all nested ternary ops
...
// Functions (normal like java) 
// Get email: slice up to specific .indexOf("@") 
function getUserEmailFunc(email) { 
  return email.slice(0, email.indexOf("@")); 
}

// Anonymous Function ( no name provided but can hold in variable) 
const getUserEmailAnonymous = function(email) { 
  return email.slice(0, email.indexOf("@"))
}
// Arrow Function no name (args) = > { } 
...
// Code for getting which email domain 
const getDomain = (email) => { 
  return email.slice(email.indexOf("@")); // returns from @ onwards
}